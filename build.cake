...
var externalFolder = System.IO.Path.Combine(workingDirectory, "external");
    ...
    try
    {
        ...
        foreach(var project in externalProjects) {
            ...
            foreach(var docName in xlfDocNames)
            {
                ...
                // load a language template
                var templateFileLocation = System.IO.Path.Combine(locExternalTemplateDir, System.IO.Path.GetFileName(docName) + ".template");
                var templateDoc = new XliffParser.XlfDocument(templateFileLocation);
                var templateFile = templateDoc.Files.Single();

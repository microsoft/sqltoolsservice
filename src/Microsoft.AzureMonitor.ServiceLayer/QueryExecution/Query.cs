...
using Microsoft.AzureMonitor.ServiceLayer.Utility;
...
namespace Microsoft.AzureMonitor.ServiceLayer.QueryExecution
{
    ...
    public class Query : IDisposable
    {
        ...
        /// </summary>
        private readonly MonitorDataSource _datasource;
        ...
        /// <param name="getFullColumnSchema"></param>
        public Query(
            string queryText,
            MonitorDataSource connection,
            ...
            bool getFullColumnSchema = false)
        {
            ...
            var parser = new BatchParserWrapper();
            List<BatchDefinition> parserResult = parser.GetBatches(queryText);

            var batchSelection = parserResult.Select((batchDefinition, index) =>
                    new Batch(batchDefinition.BatchText, new SelectionData(
        ...
        /// </summary>
        private Batch[] Batches { get; }

        /// <summary>
        /// The batches which should run after the user batches 
        /// </summary>
        private List<Batch> AfterBatches { get; }
        ...
        /// <summary>
        /// Storage for the async task for execution.
        /// </summary>
        ...
        {
            get { return Batches.Length == 0 ? _hasExecuteBeenCalled : Batches.All(b => b.HasExecuted); }
            internal set
            {
                _hasExecuteBeenCalled = value;
        ...
        {
            Logger.Write(TraceEventType.Start,
                $"Starting GetSubset execution for batchIndex:'{batchIndex}', resultSetIndex:'{resultSetIndex}', startRow:'{startRow}', rowCount:'{rowCount}'");
        ...
        private async Task ExecuteInternal()
        {
            try
            {
                ...
                {
                    await b.Execute(_datasource, _cancellationSource.Token);
                }

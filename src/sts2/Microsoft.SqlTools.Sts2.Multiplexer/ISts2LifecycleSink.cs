...
namespace Microsoft.SqlTools.Sts2.Multiplexer
{
    ...
    {
        /// <summary>
        /// Mirror of the legacy <c>shutdown</c> request. Implementations must not write
        /// any JSON-RPC response for it.
        /// </summary>
        void OnShutdown();

        /// <summary>
        /// Mirror of the legacy <c>exit</c> notification, delivered before the raw
        /// notification is forwarded to legacy. The returned task completes when STS2
        /// has flushed its journals; the multiplexer waits at most
        /// <see cref="MultiplexerOptions.ExitFlushMilliseconds"/> before forwarding anyway.
        /// </summary>
        Task OnExitAsync();
    }
}

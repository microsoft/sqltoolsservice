...
namespace Microsoft.SqlTools.ServiceLayer.ObjectExplorer.KustoModel
{
    internal partial class DatabaseTreeNode
    {
        public DatabaseTreeNode(ServerNode serverNode, string databaseName): this()
        {
            ...
            var db = new Database(serverNode.GetContextAs<KustoQueryContext>().Server, this.NodeValue);
            db.Refresh();
            ...
            if (db.State == SqlKustoState.Creating)
            {
                db = new Database(serverNode.GetContextAs<KustoQueryContext>().Server, "master");
                db.Refresh();
            }
        ...
        protected override void EnsureContextInitialized()
        {
            if (context == null)
            {
                ...
                var db = KustoObject as Database;
                if (db != null)
                {
                    context.Database = db;
                }
        ...
        {
            var kustoQueryContext = this.GetContextAs<KustoQueryContext>();
            if (IsAccessible(kustoQueryContext))
            {
        ...
        public bool IsAccessible(KustoQueryContext context)
        {
            try
            {

...
namespace Microsoft.SqlTools.ServiceLayer.ObjectExplorer.KustoModel
{
    ...
    internal partial class DatabasesChildFactory : KustoChildFactoryBase
    {
        ...
        {
            return DatabasesCustomNodeHelper.GetStatus(smoObject, smoContext, CachedKustoProperties);
        }
        ...
        protected override void InitializeChild(TreeNode parent, TreeNode child, object context)
        {
            ...
            var smoTreeNode = child as KustoTreeNode;
            if (smoTreeNode != null && smoTreeNode.KustoObject != null
                && DatabasesCustomNodeHelper.GetDatabaseIsUnavailable(smoTreeNode.KustoObject, parent.GetContextAs<KustoQueryContext>(), CachedKustoProperties))
            {
    ...
    internal static class DatabasesCustomNodeHelper
    {
        ...
        internal static bool GetDatabaseIsUnavailable(object smoObject, KustoQueryContext smoContext, IEnumerable<NodeKustoProperty> supportedProperties)
        {
            Database db = smoObject as Database;
            if (db != null && KustoChildFactoryBase.IsPropertySupported("Status", smoContext, db, supportedProperties))
            {
        ...
        internal static string GetStatus(object smoObject, KustoQueryContext smoContext, IEnumerable<NodeKustoProperty> supportedProperties)
        {
            Database db = smoObject as Database;
            if (db != null && KustoChildFactoryBase.IsPropertySupported("Status", smoContext, db, supportedProperties))
            {

...
namespace Microsoft.SqlTools.ServiceLayer.ObjectExplorer.KustoModel
{
    ...
    public class KustoQueryContext
    {
        ...
        private KustoObjectBase parent;
        ...
        public KustoQueryContext(Server server, IMultiServiceProvider serviceProvider)
            : this(server, serviceProvider, null)
        {
        }

        internal KustoQueryContext(Server server, IMultiServiceProvider serviceProvider, KustoWrapper serverManager)
        {
            this.server = server;
            ServiceProvider = serviceProvider;
            this.smoWrapper = serverManager ?? new KustoWrapper();
        }
        ...
        /// <summary>
        /// A query loader that can be used to find <see cref="KustoQuerier"/> objects
        ...
        /// <returns>new <see cref="KustoQueryContext"/> with all fields except <see cref="Parent"/> the same</returns>
        public KustoQueryContext CopyWithParent(KustoObjectBase parent)
        {
            KustoQueryContext context = new KustoQueryContext(this.Server, this.ServiceProvider, this.smoWrapper)
            {
        ...
        /// </summary>
        public void EnsureConnectionOpen(KustoObjectBase smoObj)
        {

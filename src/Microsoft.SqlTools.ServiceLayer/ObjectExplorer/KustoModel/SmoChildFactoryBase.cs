...
{
    public class KustoChildFactoryBase : ChildFactory
    {
        private IEnumerable<NodeKustoProperty> smoProperties;
        ...
        {
            KustoQueryContext context = parent.GetContextAs<KustoQueryContext>();
        ...
        protected virtual void OnExpandPopulateNonFolders(IList<TreeNode> allChildren, TreeNode parent, bool refresh, string name, CancellationToken cancellationToken)
        {
            ...
            IEnumerable<KustoQuerier> queriers = context.ServiceProvider.GetServices<KustoQuerier>(q => IsCompatibleQuerier(q));
            var filters = this.Filters.ToList();
            var smoProperties = this.KustoProperties.Where(p => ServerVersionHelper.IsValidFor(serverValidFor, p.ValidFor)).Select(x => x.Name);
        ...
        private bool ShouldFilterNode(TreeNode childNode, ValidForFlag validForFlag)
        {
            ...
            KustoTreeNode smoTreeNode = childNode as KustoTreeNode;
            if (smoTreeNode != null)
            {
        ...
        private bool IsCompatibleQuerier(KustoQuerier querier)
        {
            if (ChildQuerierTypes == null)
            {
                return false;
        ...
        {
            NamedKustoObject smoObj = context as NamedKustoObject;
            if (smoObj == null)
            {
                Debug.WriteLine("context is not a NamedKustoObject. type: " + context.GetType());
            }
            else
            {
                smoProperties = KustoProperties;
                KustoTreeNode childAsMeItem = (KustoTreeNode)child;
                childAsMeItem.CacheInfoFromModel(smoObj);
                KustoQueryContext smoContext = parent.GetContextAs<KustoQueryContext>();
        ...
        public override IEnumerable<NodeKustoProperty> KustoProperties
        {
            get
            {
                return Enumerable.Empty<NodeKustoProperty>();
            }
        }

        internal IEnumerable<NodeKustoProperty> CachedKustoProperties
        {
            get
            {
                return smoProperties == null ? KustoProperties : smoProperties;
            }
        }
        ...
        public static bool IsPropertySupported(string propertyName, KustoQueryContext context, NamedKustoObject smoObj, IEnumerable<NodeKustoProperty> supportedProperties)
        {
            ...
            else
            {
                ...
                Logger.Write(TraceEventType.Verbose, $"Kusto property name {propertyName} for Kusto type {smoObj.GetType()} is not added as supported properties. This can cause the performance of loading the OE nodes");
                return true;
            }
        }
        ...
        {
            return (smoObject as NamedKustoObject).Name;
        }
    }
}

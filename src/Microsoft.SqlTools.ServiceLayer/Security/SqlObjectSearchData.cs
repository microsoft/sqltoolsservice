...
namespace Microsoft.SqlServer.Management.SqlMgmt
{
    ...
    enum SearchableObjectType
    {
        ...
        AvailabilityGroupContained = 37,
        LastType = 38    // do not add object types after LastType - insert ahead of LastType instead
    }
    ...
    class SearchableObjectTypeCollection : System.Collections.CollectionBase
    {
        ...
        protected override void OnInsert(int index, Object value)
        {
            ...
            {
                System.Diagnostics.Debug.Fail("value is not a SearchableObjectType");
        ...
        protected override void OnSet(int index, Object oldValue, Object newValue)
        {
            ...
            {
                System.Diagnostics.Debug.Fail("newValue is not a SearchableObjectType");
                throw new ArgumentException();
            }
        }
    ...
    class SearchableObjectTypeDescription
    {
        ...
        {
            System.Diagnostics.Debug.Assert(image != null, "image is null");
            System.Diagnostics.Debug.Assert((typeNameKey != null) && (typeNameKey.Length != 0), "typeNameKey is null or empty");
            System.Diagnostics.Debug.Assert(specialRestrictions != null, "specialRestrictions is null");
            System.Diagnostics.Debug.Assert(disallowSystemObjectsRestriction != null, "disallowSystemObjectsRestriction is null");
            System.Diagnostics.Debug.Assert(resourceManager != null, "resourceManager is null");
            ...
            System.Diagnostics.Debug.Assert((this.typeNamePlural != null) && (this.typeNamePlural.Length != 0), "could not get plural type name");
            System.Diagnostics.Debug.Assert((this.typeNameSingular != null) && (this.typeNameSingular.Length != 0), "could not get singular type name");
        }
        ...
        {
            System.Diagnostics.Debug.Assert(!this.isDatabaseObject, "wrong overload for enumerating database objects");
        ...
        {
            System.Diagnostics.Debug.Assert(this.isDatabaseObject, "wrong overload for enumerating server objects");
            System.Diagnostics.Debug.Assert(databaseName.Length != 0, "database name is empty");
        ...
        {
            System.Diagnostics.Debug.Assert(!this.isDatabaseObject, "wrong overload for enumerating database objects");
            System.Diagnostics.Debug.Assert(name.Length != 0, "name is empty");
        ...
        // DEVNOTE(MatteoT, 7/17/2022). This private field seems to be used to try
        // and cache the results. Since it's static, it's initialized once and forever.
        //
        // However, if the user browse objects on a different server during the same SSMS session,
        // the data would be stale/incorrect.
        //
        // We probably never realized this was an issue since we (and the users) because in order
        // to expose it (it's crash in SSMS 18.12.1, by the way) a user would have had to:
        // 1) Connecto to SQL 2012 (or higher)
        // 2) Right click on Server node and bring up Properties
        // 3) Click on Permissions page, then click "Search..."
        // 4) Connect to SQL 2008R2 (or lower)
        // 5) Repear steps 2 and 3
        //
        // For this reason, instead of a bool, I go by version number. It is not clear if 
        // we should even more specific here to keep into account the fact that Azure DBs
        // show a version that is kind of useless...
        private static Version firstVersionSpecificTypeInfoUpdate = null;
        ...
        {
            if (SearchableObjectTypeDescription.firstVersionSpecificTypeInfoUpdate != serverVersion)
            {
                ...
                // DEVNOTE(MatteoT, 7/17/2022) - this special case could well removed and declare this 
                // SearchableObjectType using the proper attribute on the SecurableType.ServerRole enum
                ...
                // See if this particular SearchableObjectType 'type' happens to be associated with a SecurableType that
                // has MultiSearchObjectSecurable..

                // Remove all the entries we may have dynamically (from metadata) added earlier:
                // we could probably try to be more selective, but it may not be worth the trouble.
                //
                // Note: we do this because the typeToDescription dictionary is updated here and it
                //       may be updated again if/when we connect to a server with a different version
                //
                // When testing, always cover the scenarios:
                // 1) Connect to OLD server, bring up securable dialogs, connect to NEW server, bring up securable dialog
                // 2) Connect to NEW server, bring up securable dialogs, connect to OLD server, bring up securable dialog
                // and verify we don't trip over something, e.g. this method does not behave properly.
                //
                foreach (var sot in PermissionsDataExtensions.SearchableObjectTypesFromMetadata.Value)
                {
                    SearchableObjectTypeDescription.typeToDescription.Remove(sot);
                }

                foreach (var sosi in PermissionsDataExtensions.GetBestApplicableMultiSearchObjectSecurableInfoFromMetadata(serverVersion))
                {
                    SearchableObjectTypeDescription.typeToDescription[sosi.SearchableObjectType] =
                        new SearchableObjectTypeDescription(
                            DpiUtil.GetScaledIcon(ResourceUtils.LoadIcon(sosi.IconResourceName)).ToBitmap(),
                            sosi.KeyNameForLocalizedResources,
                            sosi.UrnObjectType,
                            sosi.SpecialRestrictions,
                            sosi.DisallowSystemObjectsRestriction,
                            sosi.IsDatabaseObject,
                            sosi.IsSchemaObject,
                            resourceManager);
                }

                firstVersionSpecificTypeInfoUpdate = serverVersion;
            }
        }
        ...
        static SearchableObjectTypeDescription()
        {
            ...
            // This will be specialized for SQL 2019 (and earlier) and SQL 2022 (and later) in
            // the metadata on the SecurableType enum.
            ...
            // DEVNOTE(MatteoT, 7/17/2022) TO IMPLEMENTORS:
            // - IF YOU ARE ADDING A NEW SECURABLE TYPE, ALWAYS ADD AN "STATIC" ENTRY HERE
            // - IF YOU THE SECURABLE TYPE MAPS TO MULTIPLE SEARCHOBJECT TYPES (typically a specialization
            //   of the same securable, like "Availability Group" and "Availability Group (Contained)"
            //   YOU NEED TO ADD THE PROPER ATTRIBUTES TO THE SecurableType enum TO CAPTURE THE WAY THEY
            //   ARE PRESENTED TO THE USER IN SEARCH/BROWSE/etc and/or how they may differ from version to
            //   VERSION OF SQL SERVER.
            // - IN A WAY, THINK OF THIS LIST AS A STATIC LIST WHICH MAY BE SPECIALIZED VIA METADATA
            //   AND BY KNOWING THE SERVER VERSION WHEN CONNECTED.
            // - IF YOU ARE ADDING A SECURABLE TYPE FOR A NON-SERVER OBJECT, BE AWARE THAT SOME CODE
            //   NEEDS TO BE IMPLEMENTED (YOU'LL GET ASSERTS TO GUIDE YOU IN WHAT NEEDS TO BE ADDED).
        }
    ...
    class SearchableObject : IComparable
    {
            ...
            get
            {
                ...
                if (!SearchableObjectTypeDescription.GetDescription(this.objectType).IsServerObject)
                {
                        ...
                        (0 != String.Compare(currentUrn.Type, "Database", StringComparison.OrdinalIgnoreCase)))
                    {
                        ...
                        System.Diagnostics.Debug.Assert(currentUrn != null, "ran off the top of the xpath");
                    }
            ...
            string fullObjectName)
        {
                ...
                if (typeDescription.IsDatabaseObject)
                {
                    ...
                    if (typeDescription.IsSchemaObject)
                    {
                        ...
                        else
                        {
                            ...
                            System.Diagnostics.Debug.Assert(dt.Rows.Count == 1, "unexpected number of rows returned");
                        ...
                        System.Diagnostics.Debug.Assert(schemaName.Length != 0, "schema name is empty");
            ...
            System.Diagnostics.Debug.Assert(results.Count <= 1, "more than one object found for an 'exact match' search");
        ...
        public static string GetSqlCollation(object connectionInfo, string databaseName)
        {
            ...
            System.Diagnostics.Debug.Assert(queryResults.Rows.Count == 1, "unexpected number of collations returned");
            ...
            string databaseName)
        {
            ...
            for (int i = 0; i < enumeratedUrns.Rows.Count; ++i)
            {
                ...
                System.Diagnostics.Debug.Assert(enumeratedUrn.Length != 0, "received empty urn string");

                SearchableObject enumeratedObject = new SearchableObject(type, enumeratedUrn, compareOptions);
                System.Diagnostics.Debug.Assert(!collection.Contains(enumeratedObject), "enumerated object is already in the result");
    ...
    class SearchableObjectCollectionEnumerator : IEnumerator
    {
        ...
        public void Reset()
        {
            if (!this.isValid)
            {
                System.Diagnostics.Debug.Fail("The enumerator has been invalidated.");
        ...
        public object Current
        {
            get
            {
                ...
                {
                    System.Diagnostics.Debug.Fail("The enumerator is positioned before start of collection.  Did you forget to call MoveNext()?");
                    throw new InvalidOperationException();
                }

                if (null == this.currentObject)
                {
                    System.Diagnostics.Debug.Fail("There is no current object.  Did you forget to check the result of MoveNext()?");
                    throw new InvalidOperationException();
                }

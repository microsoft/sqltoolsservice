...
namespace Microsoft.SqlServer.Management.SqlManagerUI
{
    ...
    internal interface ILogViewer
    {
        ...
        Control                 Owner {get;} // for IWin32Window used to parent various ui
    }
    ...
    internal abstract class LogSourceType: ILogSourceType
    {
        ...
        public virtual void UpdateSourceTree(LogViewerAggregatorSelection[] initialSelections, LogViewerAggregator caller)
        {
            ...
            System.Diagnostics.Debug.Assert(logSources != null, "Log Source Type returned NULL .Sources");
            if (logSources != null)
            {
                int logNumber = 0;
                foreach (ILogSource logSource in this.logSources)
                {
                    System.Diagnostics.Debug.Assert(logSource != null, "source type returned null ILogSource");
        ...
        public virtual IDictionary<ILogSource,string> GetSelectedSources(TreeNode root)
        {
            ...
            {
                System.Diagnostics.Debug.Assert(tnLogSource.Tag is ILogSource);

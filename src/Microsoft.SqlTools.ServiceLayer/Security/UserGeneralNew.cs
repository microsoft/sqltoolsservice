...
{
    public partial class UserGeneralNew : SqlManagementUserControl, IPanelForm, ISupportValidation
    {
        private UserData.UserPrototypeFactory userPrototypeFactory;
        private UserData.ExhaustiveUserTypes currentUserType;
        ...
        private UserData.UserPrototypeNew currentUserPrototype;
        ...
        public UserGeneralNew(CDataContainer context)
        {
            ...
            this.HelpF1Keyword = AssemblyVersionInfo.VersionHelpKeywordPrefix + @".swb.databaseuser.general.f1";
            this.defaultLanguagePlaceholder = UserSR.DefaultLanguagePlaceHolder;

            this.userPrototypeFactory = UserData.UserPrototypeFactory.GetInstance(this.DataContainer);

            if (this.DataContainer.IsNewObject)
            {
                if (this.IsParentDatabaseContained())
                {
                    this.currentUserType = UserData.ExhaustiveUserTypes.SqlUserWithPassword;
                }
                else
                {
                    this.currentUserType = UserData.ExhaustiveUserTypes.LoginMappedUser;
                }
            }
            else
            {
            ...
            this.PopulateUserTypeComboBox();
            this.userTypeComboBox.Text = this.GetUserTypeTextFromEnum(this.currentUserType);

            this.PopulateDefaultLanguageComboBox();

            this.InitializeValuesInUiControls();
            this.UpdateUiControlsOnLoad();
        ...
        private void InitializeValuesInUiControls()
        {
            ...
            UserData.IUserPrototypeWithMappedLogin mappedLoginPrototype = this.currentUserPrototype
                                                                            as UserData.IUserPrototypeWithMappedLogin;
            if (mappedLoginPrototype != null)
            {
                this.mappedObjTextbox.Text = mappedLoginPrototype.LoginName;
            }

            UserData.IUserPrototypeWithDefaultLanguage defaultLanguagePrototype = this.currentUserPrototype
                                                                                    as UserData.IUserPrototypeWithDefaultLanguage;
            ...
            UserData.IUserPrototypeWithDefaultSchema defaultSchemaPrototype = this.currentUserPrototype
                                                                                as UserData.IUserPrototypeWithDefaultSchema;
            ...
            UserData.IUserPrototypeWithPassword userWithPwdPrototype = this.currentUserPrototype
                                                                        as UserData.IUserPrototypeWithPassword;
        ...
        {
            UserData.IUserPrototype prototype = this.currentUserPrototype as UserData.IUserPrototype;
            UserData.IUserPrototypeWithMappedLogin mappedLoginPrototype = this.currentUserPrototype
                                                                            as UserData.IUserPrototypeWithMappedLogin;
            ...
            UserData.IUserPrototypeWithDefaultLanguage userWithDefaultLanguagePrototype = this.currentUserPrototype
                                                                                        as UserData.IUserPrototypeWithDefaultLanguage;
            ...
            UserData.IUserPrototypeWithDefaultSchema userWithDefaultSchemaPrototype = this.currentUserPrototype
                                                                                        as UserData.IUserPrototypeWithDefaultSchema;
            ...
            if (this.currentUserType == UserData.ExhaustiveUserTypes.WindowsUser)
            {
        ...
        private UserData.ExhaustiveUserTypes GetUserTypeEnumFromText(string userTypeText)
        {
            if (0 == string.Compare(userTypeText, UserSR.AsymmetricKeyUserTypeText, StringComparison.Ordinal))
            {
                return UserData.ExhaustiveUserTypes.AsymmetricKeyMappedUser;
            }
            else if (0 == string.Compare(userTypeText, UserSR.CertificateUserTypeText, StringComparison.Ordinal))
            {
                return UserData.ExhaustiveUserTypes.CertificateMappedUser;
            }
            else if (0 == string.Compare(userTypeText, UserSR.LoginMappedSqlUserTypeText, StringComparison.Ordinal))
            {
                return UserData.ExhaustiveUserTypes.LoginMappedUser;
            }
            else if (0 == string.Compare(userTypeText, UserSR.WithoutLoginSqlUserTypeText, StringComparison.Ordinal))
            {
                return UserData.ExhaustiveUserTypes.SqlUserWithoutLogin;
            }
            else if (0 == string.Compare(userTypeText, UserSR.SqlUserWithPasswordUserTypeText, StringComparison.Ordinal))
            {
                return UserData.ExhaustiveUserTypes.SqlUserWithPassword;
            }
            else if (0 == string.Compare(userTypeText, UserSR.WindowsUserTypeText, StringComparison.Ordinal))
            {
                return UserData.ExhaustiveUserTypes.WindowsUser;
            }
            else
            {
                System.Diagnostics.Debug.Assert(false, "Unknown UserType Text provided");
                return UserData.ExhaustiveUserTypes.Unknown;
            }
        }

        private string GetUserTypeTextFromEnum(UserData.ExhaustiveUserTypes userTypeEnum)
        {
            switch (userTypeEnum)
            {
                case UserData.ExhaustiveUserTypes.AsymmetricKeyMappedUser:
                    return UserSR.AsymmetricKeyUserTypeText;
                    
                case UserData.ExhaustiveUserTypes.CertificateMappedUser:
                    return UserSR.CertificateUserTypeText;
                    
                case UserData.ExhaustiveUserTypes.LoginMappedUser:
                    return UserSR.LoginMappedSqlUserTypeText;
                    
                case UserData.ExhaustiveUserTypes.SqlUserWithoutLogin:
                    return UserSR.WithoutLoginSqlUserTypeText;
                    
                case UserData.ExhaustiveUserTypes.SqlUserWithPassword:
                    return UserSR.SqlUserWithPasswordUserTypeText;
                    
                case UserData.ExhaustiveUserTypes.WindowsUser:
        ...
        private UserData.ExhaustiveUserTypes GetCurrentUserTypeForExistingUser(User user)
        {
        ...
        public override void OnGatherUiInformation(RunType runType)
        {
            ...
            if (this.currentUserType == UserData.ExhaustiveUserTypes.SqlUserWithPassword
        ...
        {
            if ((this.currentUserType == UserData.ExhaustiveUserTypes.AsymmetricKeyMappedUser
                    || this.currentUserType == UserData.ExhaustiveUserTypes.CertificateMappedUser
                    || this.currentUserType == UserData.ExhaustiveUserTypes.LoginMappedUser)
                && 0 == this.mappedObjTextbox.Text.Trim().Length)
            {
                ...
                else if (this.currentUserType == UserData.ExhaustiveUserTypes.CertificateMappedUser)
                {
                    errorMsg = UserSR.Error_SpecifyACertificate;
                }
                else if (this.currentUserType == UserData.ExhaustiveUserTypes.LoginMappedUser)
                {
        ...
        private void OnSpecifyOldPasswordCheckedChanged(object sender, EventArgs e)
        {
            ...
            (this.currentUserPrototype as UserData.IUserPrototypeWithPassword).IsOldPasswordRequired 
        ...
        private void OnPasswordTextBoxTextChanged(object sender, EventArgs e)
        {
            ...
            {
                UserData.IUserPrototypeWithPassword passwordUserPrototype = this.currentUserPrototype
                                                                            as UserData.IUserPrototypeWithPassword;
        ...
        private void OnMappedObjSearchButtonClicked(object sender, EventArgs e)
        {
            ...
            {
                UserData.IUserPrototypeWithMappedLogin mappedLoginPrototype
                    = this.currentUserPrototype as UserData.IUserPrototypeWithMappedLogin;

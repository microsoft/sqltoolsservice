...
/// </summary>
public static class VSCodeKernelFunctionMetadataExtensions
{
    /// <summary>
    /// Convert a <see cref="KernelFunctionMetadata"/> to an <see cref="VSCodeFunctionParameter"/>.
    /// </summary>
    ...
    public static OpenAIFunction ToOpenAIFunction(this KernelFunctionMetadata metadata)
    {
        ...
        var openAIParams = new VSCodeFunctionParameter[metadataParams.Count];
        for (int i = 0; i < openAIParams.Length; i++)
        {
            var param = metadataParams[i];

            openAIParams[i] = new VSCodeFunctionParameter(
                param.Name,

...
/// </summary>
public sealed class VSCodeFunctionToolCall
{
    private string? _fullyQualifiedFunctionName;

    /// <summary>Initialize the <see cref="VSCodeFunctionToolCall"/> from a <see cref="ChatToolCall "/>.</summary>
    internal VSCodeFunctionToolCall(ChatToolCall functionToolCall)
    {
    ...
    /// <summary>Initialize the <see cref="VSCodeFunctionToolCall"/> from a <see cref="ChatToolCall "/>.</summary>
    internal VSCodeFunctionToolCall(ChatTool responseTool, string responseToolParameters)
    {
        Verify.NotNull(responseTool);
        Verify.NotNull(responseTool.FunctionName);

        string fullyQualifiedFunctionName = responseTool.FunctionName;
        string functionName = fullyQualifiedFunctionName;
        string? arguments = responseToolParameters;
        string? pluginName = null;
        ...
        this.Id = responseTool.FunctionName;
        this._fullyQualifiedFunctionName = fullyQualifiedFunctionName;
        this.PluginName = pluginName;
        this.FunctionName = functionName;
        if (!string.IsNullOrWhiteSpace(arguments))

...
namespace Microsoft.SqlTools.SqlCore.TableDesigner
{
    public class TableDesignerManager
    {
        ...
        public ProcessTableDesignerEditResponse TableDesignerEdit(ProcessTableDesignerEditRequestParams requestParams)
        {
            ...
            try
            {
                switch (requestParams.TableChangeInfo.Type)
                {
                    case DesignerEditType.Add:
                        this.AddItem(requestParams);
                        break;
                    case DesignerEditType.Remove:
                        this.RemoveItem(requestParams);
                        break;
                    case DesignerEditType.Update:
                        refreshViewRequired = this.UpdateItem(requestParams);
                        break;
                    case DesignerEditType.Move:
                        this.MoveItem(requestParams);
                        break;
                    default:
                        break;
                }
        ...
        private void RemoveItem(ProcessTableDesignerEditRequestParams requestParams)
        {
            var table = this.GetTableDesigner(requestParams.TableInfo).TableViewModel;
            var path = requestParams.TableChangeInfo.Path;
            // Handle the add item request on top level table properties, e.g. Columns, Indexes.
        ...
        private bool UpdateItem(ProcessTableDesignerEditRequestParams requestParams)
        {

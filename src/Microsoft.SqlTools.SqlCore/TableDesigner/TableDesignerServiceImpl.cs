...
using Microsoft.SqlTools.SqlCore;
...
namespace Microsoft.SqlTools.ServiceLayer.TableDesigner
{
    ...
    /// </summary>
    public sealed class TableDesignerServiceImpl : IDisposable
    {
        ...
        public TableDesignerServiceImpl()
        {
        }

        public TableDesignerSettings Settings { get; set; } = new TableDesignerSettings();

        public Task UpdateSettings(SqlToolsSettings newSettings, SqlToolsSettings oldSettings, EventContext eventContext)
        {
        ...
        public ProcessTableDesignerEditResponse ProcessTableDesignerEdit(TableInfo tableInfo, TableDesignerChangeInfo tableChangeInfo)
        {
            var refreshViewRequired = false;
            string inputValidationError = null;
            DesignerPathUtils.Validate(tableChangeInfo.Path, tableChangeInfo.Type);
            try
            {
                switch (tableChangeInfo.Type)
                {
                    case DesignerEditType.Add:
                        this.HandleAddItemRequest(tableInfo, tableChangeInfo);
                        break;
                    case DesignerEditType.Remove:
                        this.HandleRemoveItemRequest(tableInfo, tableChangeInfo);
                        break;
                    case DesignerEditType.Update:
                        refreshViewRequired = this.HandleUpdateItemRequest(tableInfo, tableChangeInfo);
                        break;
                    case DesignerEditType.Move:
                        this.HandleMoveItemRequest(tableInfo, tableChangeInfo);
                        break;
                    default:
                        break;
                }
        ...
        public PublishTableChangesResponse PublishTableChanges(TableInfo tableInfo)
        {
            ...
            {
                string newId = string.Format("{0}|{1}|{2}|{3}|{4}", ProviderName, tableInfo.Server, tableInfo.Database, tableDesigner.TableViewModel.Schema, tableDesigner.TableViewModel.Name);
                if (newId != oldId)
                {
        ...
        public string GenerateScript(TableInfo tableInfo)
        {
        ...
        public void HandleAddItemRequest(TableInfo tableInfo, TableDesignerChangeInfo tableChangeInfo)
        {
            var table = this.GetTableDesigner(tableInfo).TableViewModel;
            var path = tableChangeInfo.Path;
        ...
        private void HandleRemoveItemRequest(TableInfo tableInfo, TableDesignerChangeInfo tableChangeInfo)
        {
            var table = this.GetTableDesigner(tableInfo).TableViewModel;
            var path = tableChangeInfo.Path;
            // Handle the add item request on top level table properties, e.g. Columns, Indexes.
        ...
        private bool HandleUpdateItemRequest(TableInfo tableInfo, TableDesignerChangeInfo tableChangeInfo)
        {
            var refreshView = false;
            var tableDesigner = this.GetTableDesigner(tableInfo);
            var table = tableDesigner.TableViewModel;
            var path = tableChangeInfo.Path;
            var newValue = tableChangeInfo.Value;
            if (path.Length == 1)
            {
                var propertyName = path[0] as string;
                switch (propertyName)
        ...
        private void HandleMoveItemRequest(TableInfo tableInfo, TableDesignerChangeInfo tableChangeInfo)
        {
            var table = this.GetTableDesigner(tableInfo).TableViewModel;
            var path = tableChangeInfo.Path;
            ...
            if (path.Length == 2)
            {
                ...
                var toIndex = Convert.ToInt32(tableChangeInfo.Value);
                switch (propertyName)
                {
                    case TablePropertyNames.Columns:
        ...
        private Dac.TableDesigner CreateTableDesigner(TableInfo tableInfo)
        {
            ...
            if (tableInfo.TableScriptPath == null)
            {
                ...
                connectionStringBuilder.ApplicationName = ConnectionUtilities.GetApplicationNameWithFeature(connectionStringBuilder.ApplicationName, TableDesignerServiceImpl.TableDesignerApplicationNameSuffix);
        ...
        internal Dac.TableDesigner GetTableDesigner(TableInfo tableInfo)
        {
            Dac.TableDesigner tableDesigner;

...
{
    public class LocalFileObservableTests
    { 
        ...
        /// </summary>
        [Test]
        public void LocalFileObservable_reads_entire_xel_file()
        {
        ...
        [Test]
        public void LocalFileObservable_calls_OnError_when_the_fetcher_fails()
        {
        ...
        private LocalFileTestObserver InitializeFileObserver(string filePath = null)
        {
            filePath ??= Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "Profiler", "TestXel_0.xel");
            var localFileObservable = new LocalFileObservable(() =>
            {
                return new XEFileEventStreamer(filePath);
            });
            var observer = new LocalFileTestObserver() { Observable = localFileObservable };
            localFileObservable.Subscribe(observer);
            return observer;
        }
    }

    sealed class LocalFileTestObserver : IObserver<ProfilerEvent>
    {
        public LocalFileObservable Observable { get; set; }

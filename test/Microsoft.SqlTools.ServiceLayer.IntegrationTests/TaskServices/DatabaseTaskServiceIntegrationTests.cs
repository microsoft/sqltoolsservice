...
namespace Microsoft.SqlTools.ServiceLayer.IntegrationTests.TaskServices
{
    public class DatabaseTaskServiceIntegrationTests
    {
        ...
        public async Task CreateDatabaseRequestCreatesDatabase()
        {
            ...
            CreateDatabaseResponse response = await AdminService.HandleCreateDatabaseRequest(requestParams);
        ...
        public async Task DropDatabaseRequestCreatesCompletedTaskWithIndeterminateProgress()
        {
            ...
            DropDatabaseResponse response = await ObjectManagementTestUtils.Service.HandleDropDatabaseRequest(requestParams);
        ...
        public async Task RenameDatabaseRequestCreatesCompletedTaskWithIndeterminateProgress()
        {
            ...
            RenameDatabaseResponse response = await ObjectManagementTestUtils.Service.HandleRenameDatabaseRequest(requestParams);
        ...
        [Test]
        public async Task RenameDatabaseRequestWithDropConnectionsCreatesCompletedTask()
        {
            string originalDatabaseName = GetUniqueDatabaseName("RenameTaskDbDropConnections");
            string renamedDatabaseName = originalDatabaseName + "_renamed";
            await SqlTestDb.CreateNewAsync(TestServerType.OnPrem, false, originalDatabaseName);
            databasesToDrop.Add(originalDatabaseName);
            databasesToDrop.Add(renamedDatabaseName);
        ...
        [Test]
        public async Task RenameDatabaseRequestGenerateScriptReturnsScriptAndDoesNotRename()
        {
            string originalDatabaseName = GetUniqueDatabaseName("RenameTaskDbScript");
            string renamedDatabaseName = originalDatabaseName + "_renamed";
            await SqlTestDb.CreateNewAsync(TestServerType.OnPrem, false, originalDatabaseName);
            databasesToDrop.Add(originalDatabaseName);
